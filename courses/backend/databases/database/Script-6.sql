...
INSERT INTO task (title, description, created, updated, due_date, status) VALUES
  ('Study SQL Queries', 'Practice writing SQL queries for data retrieval', 
  datetime('now'),
  datetime('now'), '2025-08-02', 'Done'),
  ('Learn Database Design', 'Study ER modeling and normalization',
  datetime('now'), 
  datetime('now'), '2025-08-10', 'Not started'),
  ('Write Unit Tests', 'Add test coverage for user authentication', 
  datetime('now'), datetime('now'), '2025-08-05', 'Not started'),
  ('Deploy Application', 'Set up production environment', 
datetime('now'), datetime('now'), '2025-08-20', 'Not started');
---Exercise 2
...
INSERT INTO user (name, email) VALUES ('Shilpa' , 'shilpa@gmail.com');
SELECT * FROM user; 
...
('Learn about SQL', 'Practice database queries', DATETIME('NOW'), DATETIME('NOW'),DATE('NOW','+7 DAY'),  (SELECT id FROM status WHERE name = 'In progress'));
SELECT * FROM task;

--3. Update the title of the task you just created to "Master SQL Basics"
--- SO THE MY OWN TASK ID IS 37
UPDATE task SET  title ='Master SQL Basics' WHERE id =37;
SELECT * FROM task WHERE id = 37;
--4. Change the due date of your task to two weeks from today
UPDATE  task SET due_date =DATE('NOW','+14 DAY') WHERE  id =37;  
SELECT * FROM task WHERE id = 37;
...
SELECT status_id FROM task WHERE id = 37;
...
SELECT u.* FROM user u LEFT JOIN  user_task ut ON u.id= ut.user_id
WHERE ut.task_id IS NULL;

-- 2. Find all tasks with a status of "Done"

select t.* FROM task t INNER  JOIN status s ON t.status_id =s.id 
WHERE s.name= 'Done';

--3. Find all overdue tasks (due_date is earlier than today)
SELECT * FROM task WHERE due_date < DATE('now');


-- PART  3: MODIFYING THE DATABASE SCHEMA
--  1. Add a new column called priority to the task table with 
----possible values: 'Low', 'Medium', 'High'. 💡 Remember to provide default values.
...
UPDATE task SET priority ='Low' WHERE id IN(2,4,6,8,10,13);
SELECT* FROM task;
UPDATE task SET priority ='High' WHERE id IN(3,7,9,12);
SELECT* FROM task;
-- 3. Create a new table called category with columns: 
---1. id (PRIMARY KEY) 2. name (e.g., "Work", "Personal", "Study") 
---3.color (e.g., "red", "blue", "green")
...
---category_id (FOREIGN KEY to category.id)
 CREATE TABLE task_category (task_id INTEGER NOT NULL,category_id INTEGER NOT NULL,
...
INNER JOIN task_category tc ON t.id = tc.task_id
...
SELECT c.name, COUNT(tc.task_id) AS total_tasks FROM category c
INNER JOIN task_category tc ON c.id = tc.category_id
...
INNER JOIN task_category tc ON ut.task_id = tc.task_id
